...
// Отобразить график
    ...
    protected void paintMarkers(Graphics2D canvas) {
        ...
        double prevY = Double.MIN_VALUE; // переменная для хранения предыдущего значения y-координаты
        for (Double[] point : graphicsData) {
            ...
            canvas.drawLine(x1, y2, x2, y1);  // Diagonal from top-right to bottom-left
            // Если текущее значение больше предыдущего, то закрашиваем маркер
            if (point[1] > prevY) {
                canvas.setColor(Color.GREEN);
                canvas.fillRect(x1, y1, sideLength, sideLength);
                canvas.setColor(Color.RED);
            }
            prevY = point[1]; // обновляем предыдущее значение y-координаты
        }
    }
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if (minX <= 0.0 && maxX >= 0.0) {
            ...
            // Draw the zero point on the X-axis
    ...
    protected void paintAxis(Graphics2D canvas) {
        ...
        if (minY <= 0.0 && maxY >= 0.0) {
            ...
            canvas.drawString("y", (float) labelPosY.getX() + 10,
                    (float) (labelPosY.getY() - boundsY.getY()));
